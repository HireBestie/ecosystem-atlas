...
export function AtlasDetailSheet() {
  ...
  const selected = useAtlasStore((s) => s.getSelectedNode());
  ...
  const neighbors =
    selected && atlas
      ? atlas.edges
          ...
          .filter((n): n is { edge: (typeof atlas.edges)[0]; other: AtlasNode } =>
            Boolean(n.other),
          )
      : [];
  ...
  return (
    <Sheet open={open} onOpenChange={setPanelOpen}>
      <SheetContent className="w-full gap-0 border-l border-border sm:max-w-md">
        {selected ? (
          <>
                ...
                {isEntity(selected) ? (
                  <>
                    <MetaRow
                      label="Type"
                      value={
                        ENTITY_TYPE_LABELS[selected.entityType] ??
                        selected.entityType
                      }
                    />
                    {selected.summary ? (
